...
//		bool			IsPet() const		{ return m_bCharType == CHAR_TYPE_PET; }
	...
	public:
		...
		bool			RefineInformation(BYTE bCell, BYTE bType, int iAdditionalCell = -1); //BYTE bCell
		...
		int				GetSameSkillBookInventory(LPITEM pItem) const;
		int				GetSameUpgradeItemsInventory(LPITEM pItem) const;
		int				GetSameStoneInventory(LPITEM pItem) const;
		int				GetSameBoxInventory(LPITEM pItem) const;
		int				GetSameEfsunInventory(LPITEM pItem) const;
		int				GetSameCicekInventory(LPITEM pItem) const;
#endif
...
#ifdef ENABLE_CHEQUE_EXWINDOW_SYSTEM
...
//		AttackLog			m_kAttackLog;
	...
	protected:
		bool	m_bChangeLook;
#ifdef ENABLE_MOUNT_TRANSMUTATION_SYSTEM
